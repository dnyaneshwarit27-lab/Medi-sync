...
if (!isset($_SESSION['temp_doctor_email'])) {
    header("Location: login.php");
    exit();
}

$email = $_SESSION['temp_doctor_email'];
$error = "";

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['verify'])) {
        $otp = $_POST['otp'];
        $result = validateOTP($email, $otp, 'DOCTOR_LOGIN');

        if ($result['success']) {
            // Success! Set session
            $_SESSION['doctor_id'] = $_SESSION['temp_doctor_id'];
            $_SESSION['doctor_email'] = $email;

            // Cleanup temp session
            unset($_SESSION['temp_doctor_email']);
            unset($_SESSION['temp_doctor_id']);

            logActivity('Login - Fully Authenticated', 'DOCTOR', $_SESSION['doctor_id'], "Successful 2FA login for $email");
            ...
            // Log failed attempt
            $stmt = $pdo->prepare("UPDATE otp_codes SET attempts = attempts + 1 WHERE identifier = ? AND type = 'DOCTOR_LOGIN' AND status = 'PENDING'");
            $stmt->execute([$email]);
        }
    }
    elseif (isset($_POST['resend'])) {
        $otp = generateOTP($email, 'DOCTOR_LOGIN');
        // Re-send email logic here
...
<head>
    ...
    <title>Verify OTP | MediSync</title>
    ...
    <div class="max-w-md w-full bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl p-10 relative overflow-hidden transition-colors border border-transparent dark:border-slate-800">
        ...
        <div class="relative z-10">
            <div class="mb-10 text-center">
                ...
                <h2 class="text-3xl font-bold text-slate-900 dark:text-white">Security Check</h2>
                <p class="text-slate-500 dark:text-slate-400 mt-2 font-medium">Verification code sent to<br><span class="font-bold text-blue-600 dark:text-blue-400"><?php echo $email; ?></span></p>
            </div>
            ...
            <form method="POST" class="space-y-8">
                ...
                <button type="submit" name="verify" class="w-full bg-blue-600 text-white py-5 rounded-2xl font-bold shadow-xl shadow-blue-500/20 hover:bg-blue-700 hover:-translate-y-0.5 transition-all active:scale-95 text-lg">
                    Verify & Authorize
                </button>
            </form>

            <form method="POST" class="mt-10 text-center">
                <p class="text-[10px] text-slate-400 dark:text-slate-400 uppercase tracking-widest mb-4">Didn't receive the code?</p>
                <button type="submit" name="resend" class="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:text-blue-700 uppercase tracking-widest transition-colors">Resend New OTP</button>
            </form>

            <div class="mt-8 text-center pt-8 border-t border-slate-50 dark:border-slate-800">
                <a href="login.php" class="text-[10px] font-bold text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors uppercase tracking-widest flex items-center justify-center gap-2">
                    <i class="fas fa-arrow-left"></i> Change Account
                </a>
            </div>
        </div>
    </div>
